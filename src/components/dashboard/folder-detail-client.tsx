...
import { updateFolder, deleteFolder } from "@/lib/supabase/folders";
import { createItem, updateItem, deleteItem } from "@/lib/supabase/items";
...
export function FolderDetailClient({
  ...
  folderId: string;
  initialData: FolderDetailInitialData;
}) {
  const router = useRouter();

  const [folder] = useState<Folder>(initialData.folder);
  const [items] = useState<Item[]>(initialData.items);
  ...
  const handleSaveItem = async (data: Omit<CreateItemData, "folder_id">) => {
    if (editingItem) {
      ...
      await createItem({ ...data, folder_id: folderId });
