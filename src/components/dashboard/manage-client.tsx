...
export function ManageClient({
  ...
  initialData: ManageInitialData;
}) {
  const folders = initialData.folders;
  const [items, setItems] = useState<ItemWithFolder[]>(initialData.items);
  ...
  const handleLoadMore = async () => {
    ...
    setLoadingMore(true);
    try {
      const page = await getItemsWithFoldersPage({ range, cursor: nextCursor });
    ...
    <div>
      ...
      <div className="flex flex-col gap-6">
        ...
        <section className="card-base" style={{ padding: 24 }}>
          ...
          {nextCursor && filteredItems.length === items.length && (
